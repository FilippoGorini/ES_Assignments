...
int parse_byte(parser_state* ps, char byte) {
    switch (ps->state) {
        case STATE_DOLLAR:                          // Waiting for the dollar
            if (byte == '$') {
                ps->state = STATE_TYPE;
                ps->index_type = 0;
            }
        ...
        case STATE_TYPE:                            // Reading the message type
            if (byte == ',') {                      // Done reading message type
                ...
                return NEW_MESSAGE;
            } else {                                // Message type string not complete yet
                ps->msg_type[ps->index_type] = byte;// ok!
                ps->index_type++;                   // increment for the next time;
            }
            break;
        case STATE_PAYLOAD:                         // Reading payload  
            if (byte == '*') {                      // Done reading payload
                ps->state = STATE_DOLLAR;           // get ready for a new message
